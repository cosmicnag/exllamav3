...
**Branch** [`deploy/exllamav3-kvo`](https://github.com/cosmicnag/exllamav3/tree/deploy/exllamav3-kvo) · **Base** upstream `dev` @ `f94d75ee` · **Delta** 2 commits, **Python only** (+ README commits on top)
...
- Branch `deploy/exllamav3-kvo` = upstream `dev` @ `f94d75ee` + `b7dcfa62` (the port) + `4fb7b96c` (`page_size` lookup fix in `QSAIndexer.sparse_attend` for quant caches). **`4fb7b96c` is the code tip** — README commits sit on top of it, so diff and deploy against `4fb7b96c`, not against the moving branch tip. The branch is rebased onto upstream `dev` rather than merged, so a rebase moves the base and rewrites the two commits above it.
...
- **The 2 commits are pure Python.** If your build is a *non-editable* install of base `f94d75ee`, copying those 8 files into site-packages *is* the deploy — no CUDA extension rebuild. Confirm with `git diff --name-only f94d75ee..4fb7b96c | grep -v '\.py$'` (empty) and by checking that no `ext.*` symbol appears only on the `+` side of the Python diff. **That only holds for the KVO commits themselves** — if your install sits on an *older* base you must also move the base, and upstream's base moves do change C++ (`e3b52f47`→`f94d75ee` touched 11 files under `exllamav3_ext`, so that hop needs a full rebuild; the `8271af4e`→`e3b52f47` hop was 12 `.py` files and no C++). Copy the upstream `.py` files in your base's range either way.
